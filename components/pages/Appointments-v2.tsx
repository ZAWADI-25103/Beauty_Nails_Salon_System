...
export default function AppointmentsV2() {
	...
	const [selectedMethod, setSelectedMethod] = useState<"mobile" | "card">(
		"mobile",
	);
	...
	const { createAppointment, isLoading: appointmentLoading } =
		useAppointments();
										...
										{timeSlots.map((time) => {
											...
											return (
												<button
													...
													className={`px-3 sm:px-4 py-3 rounded-xl border transition-all text-lg sm:text-base ${
														selectedTime === time
						...
						<Card className="p-4 sm:p-6 lg:p-8 border border-pink-100 dark:border-pink-900 shadow-xl rounded-2xl sticky top-24 bg-white dark:bg-gray-950">
							...
							<div className="space-y-4 mb-6">
								...
								{selectedTime && (
									<RadioGroup
										value={decideToPay}
										onValueChange={(value: any) => setDecideToPay(value)}
									>
										<div className="space-y-4">
											<div className="flex items-start sm:items-center space-x-3 p-3 sm:p-4 border border-gray-200 dark:border-gray-700 rounded-xl hover:border-pink-300 dark:hover:border-pink-600 transition-colors">
												<RadioGroupItem
												...
												>
													...
													<div>
														...
														<p className="text-base sm:text-lg text-gray-500 dark:text-gray-400">
															Faites vos payements en ligne avec nos ligne de
															payements tres securisees.
														</p>
													</div>
												</Label>
											</div>
											...
											<DialogHeader>
												<DialogTitle className="text-2xl  flex items-center gap-2 text-white">
													{decideToPay
														...
														: "Nouveau fiche de paie"}
												</DialogTitle>
												<p className="text-pink-100 opacity-90">
													{decideToPay
														? "Gérez les détails du fiche de paie existant."
														: "Confirmez le payement et les détails du rendez-vous pour une nouvelle séance beauté."}
												</p>
											</DialogHeader>
										</div>
												...
												<div className="grid grid-cols-2 gap-3">
													{Object.entries(PAYMENT_DETAILS).map(([key, val]) => (
														<Button
															key={key}
															variant={
																selectedMethod === key ? "default" : "outline"
															}
															onClick={() =>
																setSelectedMethod(
																	key === "mobile" ? "mobile" : "card",
																)
															}
														>
															{val.label}
