...
import {
	...
	loadBookingProgress,
	saveBookingProgress,
} from "@/lib/local/booking-storage";
...
export default function AppointmentsV3() {
	...
	const paymentInfo = useMemo(
		() => ({
			...
			total: isFreeService
				? 0
				: refBonus > 0
					? totalCost - (totalCost * refBonus) / 100
					: totalCost, // Zero out if free service
	...
	}, [selectedMethod, payerPhone, total, selectedServiceId, selectedWorker]);
									...
									return (
										<Card
											...
											onClick={() => {
												if (isActive) {
													setActiveAddOns(
														activeAddOns.filter((id) => id !== addOn.id),
													);
												} else {
								...
								{[
									...
									return (
										<button
											...
											disabled={disabled}
											onClick={() =>
												!disabled && setSelectedMethod(method.key as any)
											}
											className={`
							...
							<div className="pt-4 border-t border-white/10">
								<div className="flex justify-between items-center">
									...
									<span className="text-2xl font-bold bg-linear-to-r from-pink-400 to-purple-400 bg-clip-text text-transparent">
										{isFreeService
											? "0 CDF"
											: `${refBonus > 0 ? (totalCost - (totalCost * refBonus) / 100).toLocaleString() : totalCost.toLocaleString()} CDF`}
									</span>
								</div>
