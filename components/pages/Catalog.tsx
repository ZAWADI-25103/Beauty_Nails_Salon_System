									...
									>
										...
										<div className="h-64 relative overflow-hidden m-3 rounded-[2rem]">
											...
											<div className="absolute top-3 right-3">
												<Badge
													className={`backdrop-blur-md border-0 py-1.5 px-4 rounded-full shadow-lg font-bold ${
											...
											<div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
												<Button
													size="icon"
													className="bg-white text-black hover:bg-gray-100 rounded-full h-12 w-12 shadow-2xl scale-90 group-hover:scale-100 transition-transform duration-300"
												>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
								{packages.map((pkg) => {
									...
									return (
										<Card
											key={pkg.id}
											className="p-6 bg-white dark:bg-gray-900 border-b border-pink-100 dark:border-pink-900 shadow-xl rounded-3xl overflow-hidden relative transform hover:scale-[1.02] transition-transform"
										>
                        ...
                        Durée: {pkg || 'N/A'}
											...
											<div className="space-y-3 mb-8 bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-gray-100 dark:border-gray-700">
												...
												{pkg.services?.slice(0, 3).map(
													(
														service: any,
														idx: number, // Show first 3 services
													) => (
														<div
															key={idx}
						...
						<TabsContent value="promotions" className="space-y-12">
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
								{discounts.map((promo) => (
									<Card
										key={promo.id}
										className="p-6 bg-linear-to-r from-amber-50 to-orange-50 dark:from-gray-900 dark:to-gray-800 border-b border-amber-100 dark:border-amber-900 shadow-xl rounded-3xl overflow-hidden relative transform hover:scale-[1.02] transition-transform"
									>
										<div className="flex items-start justify-between mb-6">
											<div>
