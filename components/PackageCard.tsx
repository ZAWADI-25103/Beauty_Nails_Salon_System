...
export default function PackageCard({ pkg }: PackageCardProps) {
  ...
  return (
    <Card className="p-6 border border-pink-100 dark:border-pink-900/30 bg-white dark:bg-gray-950 hover:shadow-lg transition-shadow">
      ...
      {/* Services included */}
      <div className="mb-4">
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Inclus :</p>
        <ul className="space-y-2">
          {pkg.services.map((service) => (
            <li key={service.id} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
      ...
      {/* Action button */}
      <Button
        onClick={() => router.push(`/appointments?package=${pkg.id}&price=${pkg.price}`)}
