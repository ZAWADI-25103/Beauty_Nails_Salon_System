...
function HeroSection({
	...
	const currentService =
		services && services.length > 0 ? services[currentIndex] : null;
	const imageSrc = currentService
		? currentService.image || currentService.imageUrl || imageUrl
		: imageUrl;
						...
						<div className="flex flex-col sm:flex-row gap-4">
							{links?.map((link: any, index: any) => (
								<Link key={index} href={link.href}>
									<Button
										className="bg-linear-to-br from-gray-900 via-pink-800 to-pink-600 hover:from-pink-600 hover:via-pink-800 hover:to-gray-900 text-white rounded-2xl"
										size={"lg"}
									>
										{link.label}
									</Button>
					...
					{showCarousel && services && services.length > 1 && (
						<div className="mt-6 flex items-center gap-4">
							...
							<div className="ml-auto flex items-center gap-2">
								...
								<button
									onClick={() =>
										setCurrentIndex((i) => (i + 1) % (services?.length ?? 1))
									}
									className="text-white/80 hover:text-white"
