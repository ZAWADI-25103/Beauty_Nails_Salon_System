...
export default function ReportsAnalytics() {
	...
	const { data: revenueReport, isLoading: revenueLoading } =
		useRevenueReport(revenueParams);
	const { data: servicesResp, isLoading: servicesLoading } =
		useServicePerformance(period);
	const { data: clientsResp, isLoading: clientsLoading } =
		useClientAnalytics(period);
		...
		return [
			{
				frequency: "Nouveaux",
				clients: newClients,
				percentage: Math.round((newClients / total) * 100),
			},
			{ frequency: "Retenus", clients: retained, percentage: retainedPct },
			{
				...
				percentage: Math.max(
					0,
					100 - Math.round((newClients / total) * 100) - retainedPct,
				),
			},
		];
	}, [clientsResp]);
	...
	const { data: staffResp, isLoading: staffLoading } =
		useStaffReport(staffParams);
	const { data: peakResp, isLoading: peakLoading } =
		usePeakHours(peakHoursParams);
	const { data: membershipResp, isLoading: membershipLoading } =
		useMembershipAnalytics(membershipParams);
	const { data: marketingResp, isLoading: marketingLoading } =
		useMarketingCampaigns(marketingParams);

	const staffUtilization = (staffResp as any)?.workers ?? [];
	const peakHours = (peakResp as any)?.peakHours ?? [];
	const membershipAnalytics = (membershipResp ??
		(clientsResp && {
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				<Card className="p-4 sm:p-6 hover:shadow-lg transition-all border border-pink-100 hover:border-pink-400 dark:border-pink-900 dark:hover:border-pink-400 shadow-xl rounded-2xl bg-linear-to-br from-green-50 to-emerald-50 dark:from-gray-900 dark:to-gray-800">
					...
					<p className="text-2xl sm:text-3xl font-medium  text-gray-900 dark:text-gray-100">
						{revenueReport ? (
							...
							revenueReport?.totalRevenue > 5000000 ? (
				...
				<Card className="p-4 sm:p-6 hover:shadow-lg transition-all border border-pink-100 hover:border-pink-400 dark:border-pink-900 dark:hover:border-pink-400 shadow-xl rounded-2xl bg-linear-to-br from-amber-50 to-orange-50 dark:from-gray-900 dark:to-gray-800">
					...
					<p className="text-2xl sm:text-3xl font-medium  text-gray-900 dark:text-gray-100">
						{clientsResp ? (
							`${Math.round(clientsResp.retentionRate)}%`
						) : (
							<Loader2 className="w-5 h-5 animate-spin text-purple-500 dark:text-purple-400 mx-auto mb-4" />
						)}
					</p>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{servicesLoading ? (
								...
								services.map((service: any, idx: any) => (
									<Card
										key={idx}
										className="p-5 sm:p-6 border border-pink-50 dark:border-pink-900/30 bg-linear-to-r from-purple-50 to-pink-50 dark:from-gray-800 dark:to-gray-800/50 rounded-xl hover:shadow-md transition-all"
									>
										...
										<div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-5">
											<div
												...
												style={{
													width: `${(service.revenue / 13500000) * 100}%`,
												}}
											/>
										</div>
									</Card>
								...
								<ResponsiveContainer width="100%" height="100%">
									<PieChart>
										<Pie
											...
											labelLine={false}
											label={(entry: any) =>
												`${entry.frequency}: ${entry.percentage}%`
											}
											outerRadius={100}
											fill="#8884d8"
											dataKey="clients"
										>
											{clientFrequency.map((entry, index) => (
												<Cell
													key={`cell-${index}`}
													fill={
														["#ec4899", "#a855f7", "#f59e0b", "#10b981"][index]
													}
												/>
											))}
										</Pie>
							...
							<div className="space-y-4">
								...
								<div className="bg-linear-to-br from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-800/50 p-5 rounded-xl border border-green-100 dark:border-green-900/30">
									...
									<p className="text-2xl sm:text-3xl font-medium  text-gray-900 dark:text-gray-100 mb-2">
										...
										membershipAnalytics.memberRevenue > 5000000
									...
									<p className="text-lg font-medium text-pink-700 dark:text-pink-300">
										...
										<span className="text-lg ">
											{membershipAnalytics.averageMemberSpend > 0
												? Math.round(
														(membershipAnalytics.averageMemberSpend /
															Math.max(
																1,
																membershipAnalytics.averageNonMemberSpend,
															)) *
															100,
													)
												: 0}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{staffLoading ? (
								...
								staffUtilization.map((staff: any, idx: any) => (
									<Card
										key={idx}
										className="p-5 sm:p-6 border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 rounded-xl hover:shadow-md transition-all"
									>
										<div className="flex items-center justify-between mb-4">
											<div>
											...
											<div className="text-right">
												<p className="text-2xl  text-pink-600 dark:text-pink-400">
													{staff.utilization || 0}%
												</p>
								...
								<div className="p-4 bg-white dark:bg-gray-950 rounded-xl shadow-sm border border-blue-50 dark:border-blue-900/20">
									<p className="text-3xl  text-blue-600 dark:text-blue-400">
										{staffUtilization.length > 0
											? Math.round(
													staffUtilization.reduce(
														(sum: any, s: any) => sum + (s.utilization || 0),
														0,
									...
									<p className="text-base text-gray-600 dark:text-gray-400 uppercase mt-1 tracking-wider">
										Taux Moyen
									</p>
								</div>
								<div className="p-4 bg-white dark:bg-gray-950 rounded-xl shadow-sm border border-blue-50 dark:border-blue-900/20">
								...
								<div className="p-4 bg-white dark:bg-gray-950 rounded-xl shadow-sm border border-blue-50 dark:border-blue-900/20">
									<p className="text-2xl  text-blue-600 dark:text-blue-400">
										{staffUtilization.length > 0
											? Math.round(
													staffUtilization.reduce(
														(sum: any, s: any) => sum + (s.totalRevenue || 0),
														0,
													) /
														staffUtilization.length /
														1000,
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{marketingLoading ? (
								...
								marketingCampaigns.map((campaign: any, idx: any) => (
									<Card
										key={idx}
										className="p-5 sm:p-6 border border-green-50 dark:border-green-900/30 bg-linear-to-r from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-800/50 rounded-xl hover:shadow-md transition-all"
									>
										<div className="flex items-center justify-between mb-4">
											<div>
												<p className="text-lg  text-gray-900 dark:text-gray-100">
											...
											<Badge className="bg-green-600 text-white px-4 py-2">
												...
												{campaign.revenue
													? Math.round(
															(campaign.revenue / (campaign.cost || 1)) * 100,
														)
													: 0}
												%
											</Badge>
						...
						<div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
							<Card className="bg-linear-to-br from-purple-50 to-pink-50 dark:from-purple-900/10 dark:to-pink-900/10 border border-purple-100 dark:border-purple-900/30 p-6 rounded-xl">
								...
								<p className="text-3xl font-black text-gray-900 dark:text-gray-100">
									{marketingCampaigns.reduce(
										(sum: any, camp: any) => sum + (camp.conversions || 0),
										0,
									)}
								</p>
							</Card>
							<Card className="bg-linear-to-br from-blue-50 to-cyan-50 dark:from-blue-900/10 dark:to-cyan-900/10 border border-blue-100 dark:border-blue-900/30 p-6 rounded-xl">
								<p className="text-lg font-medium text-gray-600 dark:text-gray-400 mb-1">
									Total Revenus Marketing
								</p>
								<p className="text-2xl font-black text-gray-900 dark:text-gray-100">
									{(
										marketingCampaigns.reduce(
											(sum: any, camp: any) => sum + (camp.revenue || 0),
											0,
										) / 1000000
									).toFixed(1)}
									M CDF
			...
			<Card className="border border-indigo-100 dark:border-indigo-900/50 shadow-xl rounded-2xl p-6 sm:p-10 bg-linear-to-br from-indigo-50/50 to-purple-50/50 dark:from-indigo-950/20 dark:to-purple-950/20">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
					...
					<Button
						...
						className="rounded-full py-7  border-indigo-100 dark:border-indigo-900 dark:text-gray-300 dark:hover:bg-indigo-900/20 bg-white dark:bg-gray-900/50"
					>
						<Users className="w-5 h-5 inline-block mr-2 text-purple-500" />{" "}
						Clientes
					</Button>
					<Button
						variant="outline"
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
					...
					<Button
						...
						className="rounded-full py-7  border-indigo-100 dark:border-indigo-900 dark:text-gray-300 dark:hover:bg-indigo-900/20 bg-white dark:bg-gray-900/50"
					>
						<Clock className="w-5 h-5 inline-block mr-2 text-green-500" />{" "}
						Heures de Pointe
					</Button>
				</div>
			</Card>
