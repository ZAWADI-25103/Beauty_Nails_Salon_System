...
export default function BookingCalendar() {
	...
	const processedAppointments = useMemo(() => {
		...
		return allAppointments.map((apt) => ({
			...
			clientName:
				apt.client?.user?.name || apt.client?.name || apt.client || "Client",
	...
	const appointmentsByDayAndStaff = useMemo(() => {
		...
		daysOfWeek.forEach((day) => {
			...
			filteredStaff.forEach((staff) => {
				grouped[day.isoDate][staff.id] = processedAppointments.filter(
					(apt) =>
						apt.date.toString().split("T")[0] === day.isoDate &&
				...
				<div className="overflow-x-auto">
					<div className="min-w-max">
						...
						<div
							...
							style={{
								gridTemplateColumns: `120px repeat(${filteredStaff.length}, minmax(200px, 1fr))`,
							}}
						>
						...
						<div className="space-y-1">
							{daysOfWeek.map((day) => (
								...
								>
									...
									{filteredStaff.map((staff) => {
										const appointments =
											appointmentsByDayAndStaff[day.isoDate]?.[staff.id] || [];
										...
										return (
											<Popover
												...
												open={
													popoverOpen.open &&
													popoverOpen.day === day.isoDate &&
													popoverOpen.staffId === staff.id
												}
												onOpenChange={(open) => {
													if (open) {
														setPopoverOpen({
                            ...
                            `}
												...
												<PopoverContent
													className="w-80 p-0 border border-pink-100 hover:border-pink-400  dark:border-pink-900 dark:hover:border-pink-400 shadow-xl rounded-2xl bg-white dark:bg-gray-950"
													align="start"
												>
													<div className="p-4">
