...
export default function ClientManagement() {
	...
	const apiClients = allClients.filter((c) => {
		const clientHasAppointmentsWithCurrentWorker = appointments.some(
			(apt) => apt.clientId === c.id && apt.workerId === user?.id,
		);
						...
						{filteredClients.map((client) => (
							...
							>
								<div className="flex items-center justify-between mb-2">
									...
									<Badge
										className={`${
											client.membershipStatus === "VIP"
												? "bg-amber-500"
												: client.membershipStatus === "Premium"
													? "bg-purple-500"
													: "bg-gray-500"
									...
									<div className="space-y-4">
										...
										<div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
											<div
												...
												style={{
													width: `${selectedClient.loyaltyPoints < 500 ? (selectedClient.loyaltyPoints / 500) * 100 : selectedClient.loyaltyPoints < 1000 ? (selectedClient.loyaltyPoints / 1000) * 100 : (selectedClient.loyaltyPoints / 1500) * 100}%`,
												}}
											/>
										</div>
