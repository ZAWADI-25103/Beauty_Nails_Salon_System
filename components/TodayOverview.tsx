...
export default function TodayOverview() {
	...
	const ongoingAppointments = apiAppointments.filter((a) =>
		["in_progress"].includes(a.status),
	);
	// console.log(ongoingAppointments[0])

	const busyRosterIds = apiAppointments
		.filter((a) => a.status === "in_progress")
		.map((apt) => apt.workerId);
		...
		apiStaff && apiStaff.length > 0
			? apiStaff
					...
					.map((s: any) => ({
						...
						service: null,
						nextAvailable: `Dans ${s.currentlyWorking.duration} minutes`,
					}))
			: [];
	...
	const lowStock = (apiInventory || []).filter(
		(i: any) =>
	...
	const popularServices =
		...
		servicePerformance.services.length > 0
			? servicePerformance.services.slice(0, 4).map((s: any) => ({
					name: s.name,
					count: s.count || 0,
					revenue: `${(s.revenue || 0).toLocaleString()} CDF`,
				}))
			: [];
	...
	return (
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<Card
					className={`border-0 shadow-lg p-5 sm:p-6 transition-all duration-300 hover:shadow-xl dark:border ${
			...
			<Card className="border-0 shadow-lg rounded-2xl p-5 sm:p-6 bg-white dark:bg-gray-950 dark:border dark:border-pink-900/30">
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
					...
					<Badge
						className={`${
							todayStats.currentOccupancy >= 80
								? "bg-red-500 dark:bg-red-900/40 text-white dark:text-red-200"
								: todayStats.currentOccupancy >= 60
									? "bg-amber-500 dark:bg-amber-900/40 text-white dark:text-amber-200"
						...
						>
							<div className="flex items-center justify-between mb-4">
								...
								<Badge
									className={`${
										staff.status === "busy"
											? "bg-blue-500 dark:bg-blue-900/40 text-white dark:text-blue-200"
											: staff.status === "available"
												? "bg-green-500 dark:bg-green-900/40 text-white dark:text-green-200"
												: "bg-amber-500 dark:bg-amber-900/40 text-white dark:text-amber-200"
					...
					<div className="space-y-3">
						{urgentAlerts.map((alert, idx) => {
							...
							return (
								<div
									key={idx}
									className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-white dark:bg-gray-900/50 rounded-xl border border-red-100 dark:border-red-900/20 shadow-sm"
								>
									<div className="flex items-center gap-4">
										<div
