...
import { Search, Package, Droplet, Scale, AlertCircle } from 'lucide-react';
...
export default function InventorySelectionModal({
  ...
  const consumables = filteredItems.filter(item => 
    ['ml', 'g', 'kg'].includes(item.unit) || item.name.toLowerCase().includes('coton')
  );
  
  const sharedResources = filteredItems.filter(item => 
    item.unit === 'pièce' && (
  ...
  const tools = filteredItems.filter(item => 
    !consumables.includes(item) && !sharedResources.includes(item)
  );
  
  const handleToggleItem = (item: any) => {
    const isShared = item.unit === 'pièce' && (
      item.name.toLowerCase().includes('flacon') ||
      item.name.toLowerCase().includes('bouteille')
    );
  ...
  const renderInventorySection = (title: string, items: any[], icon: React.ReactNode) => (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                    <Badge variant="outline" className="text-[10px]">
                      {item.unit === 'ml' && <Droplet className="w-3 h-3 mr-1" />}
                      {item.unit === 'g' || item.unit === 'kg' && <Scale className="w-3 h-3 mr-1" />}
                      {item.unit}
                    </Badge>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Sélectionnez les consommables et outils utilisés pendant ce rendez-vous
          </p>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          {sharedResources.length > 0 && renderInventorySection(
          ...
          {/* Consumables Section */}
          {consumables.length > 0 && renderInventorySection(
            'Consommables',
            consumables,
          ...
          {/* Tools Section */}
          {tools.length > 0 && renderInventorySection(
            'Outils',
            tools,
          ...
          {selectedItems.length > 0 && (
            <Card className="p-4 bg-gray-50 dark:bg-gray-800/50">
              <h4 className="font-medium mb-3">Articles sélectionnés ({selectedItems.length})</h4>
              <div className="space-y-2">
                {selectedItems.map(item => (
                  <div key={item.id} className="flex items-center justify-between text-sm">
                    <span>{item.name}</span>
                    <div className="flex items-center gap-2">
                      {!item.isShared && <span className="text-gray-500">x{item.quantity}</span>}
          ...
          <div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-sm text-blue-800 dark:text-blue-200">
            ...
            <p>
              <strong>Note:</strong> Les ressources partagées (flacons, pots) ne sont déduites qu'une fois par rendez-vous, 
              même si plusieurs esthéticiennes les utilisent. Les consommables (coton, vernis en ml) sont déduits selon la quantité utilisée.
            </p>
          </div>
        </div>
        ...
        <DialogFooter className="gap-2">
          ...
          >
            {recordUsage.isPending ? 'Enregistrement...' : `Enregistrer (${selectedItems.length} articles)`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
