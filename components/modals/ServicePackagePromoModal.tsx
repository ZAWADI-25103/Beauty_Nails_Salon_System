...
import type { CreatePackageData, ServicePackage } from "@/lib/api/packages";
import type { CreateServiceData, Service } from "@/lib/api/services";
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div className="space-y-3">
							...
							<div className="relative">
								...
								/>
								<span className="absolute left-3 top-2.5 text-base text-gray-500">
									Fr
								</span>
							</div>
						</div>
						...
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(checked) =>
									handleChange("onlineBookable", checked)
								}
							/>
						</div>
...
export function PackageModal({ pkg, trigger, onSubmit }: PackageModalProps) {
	...
	const [selectedServiceIds, setSelectedServiceIds] = useState<string[]>(
		pkg?.services?.map((s: Service) => s.id) || [],
	);
	...
	const { createPackage, updatePackage, isCreating, isUpdating } =
		usePackages();
	...
	const handleSubmit = () => {
		...
		if (pkg) {
			updatePackage({
				id: pkg.id,
				updates: submitData as Partial<ServicePackage>,
			});
		} else {
...
export function PromoModal({ promo, trigger, onSubmit }: PromoModalProps) {
	...
	const { createDiscount, updateDiscount, isCreating, isUpdating } =
		useDiscounts();
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						<div className="space-y-3">
							...
							<div className="relative">
								...
								/>
								<span className="absolute left-3 top-2.5 text-base text-amber-500">
									#
								</span>
							</div>
						</div>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<div className="space-y-3">
								...
								<Input
									...
									onChange={(e) =>
										handleChange("maxUses", parseInt(e.target.value) || 1)
									}
									className="h-11 text-base"
								/>
							</div>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							<div className="space-y-3">
								...
								<Input
									...
									onChange={(e) =>
										handleChange("minPurchase", parseFloat(e.target.value) || 0)
									}
									className="h-11 text-base"
								/>
							</div>
