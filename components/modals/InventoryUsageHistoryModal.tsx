...
import { Calendar, User, FileText, Package, Droplet, Scale } from 'lucide-react';
...
export default function InventoryUsageHistoryModal({
  ...
  const { data, isLoading } = useItemUsageHistory(
    itemId,
    dateParams
  );
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {data?.stats && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Card className="p-4 text-center">
                <div className="text-2xl font-bold text-pink-600">{data.stats.totalUsed}</div>
                <div className="text-xs text-gray-500">Total utilisé</div>
              </Card>
              <Card className="p-4 text-center">
                <div className="text-2xl font-bold text-blue-600">{data.stats.uniqueAppointments}</div>
                <div className="text-xs text-gray-500">Rendez-vous</div>
              </Card>
              <Card className="p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">{data.stats.uniqueWorkers}</div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Input placeholder="Rechercher un rendez-vous..." className="flex-1" />
          </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <Badge variant="outline" className="text-[10px]">
                          {usage.item.unit === 'ml' && <Droplet className="w-3 h-3 mr-1" />}
                          {usage.item.unit === 'g' && <Scale className="w-3 h-3 mr-1" />}
                          {usage.item.unit}
                        </Badge>
                        <span className="font-medium">{usage.quantity} {usage.item.unit}</span>
                      </div>
                      
                      {usage.usedFor && (
                        <div className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300 mb-1">
                          <FileText className="w-4 h-4" />
                          <span>RDV #{usage.usedFor.slice(-6)} - {getAppointmentServiceName(usage.usedFor)}</span>
                        </div>
                      )}
                      ...
                      {usage.usedBy && (
                        <div className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300">
                          ...
                          <span>Utilisé par: {getWorkerName(usage.usedBy)}</span>
                        </div>
                      )}
