...
export default function ClientModal({
	...
	const { user } = useAuth();
	const { createClient, updateClient, isCreatingClient, isUpdatingClient } =
		useClients();
	...
	const handleAddFavoriteService = () => {
		if (
			newFavoriteService.trim() &&
			!formData.favoriteServices.includes(newFavoriteService.trim())
		) {
			setFormData((prev) => ({
				...prev,
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div>
							...
							<Select
								...
								onValueChange={(value) =>
									handleInputChange("membershipStatus", value as any)
								}
								disabled={!canEditAllFields}
							>
						...
						<div>
							...
							<Input
								...
								onChange={(e) =>
									handleInputChange("referrals", Number(e.target.value))
								}
								disabled={!canEditAllFields}
								className="mt-1"
							/>
					...
					<div>
						...
						<div className="flex flex-wrap gap-2 mt-2">
							{formData.allergies.map((allergy, index) => (
								<Badge
									key={index}
									className="flex items-center gap-1 bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-200"
								>
									{allergy}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										handleInputChange("giftCardBalance", Number(e.target.value))
									}
									className="mt-1"
								/>
							</div>
					...
					<div className="flex justify-end gap-3 pt-4">
						...
						<Button
							type="submit"
							disabled={
								isSubmitting || (!canEditAllFields && !canEditLimitedFields)
							}
						>
							{isSubmitting
								? "Enregistrement..."
