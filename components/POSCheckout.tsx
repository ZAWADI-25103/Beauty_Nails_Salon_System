...
export default function POSCheckout({ showMock }: { showMock?: boolean }) {
	...
	const services: ItemSource[] =
		apiServices && apiServices.length
			? (apiServices as any as ItemSource[])
			: showMock
				? MOCK_SERVICES
				: [];
	const products: ItemSource[] =
		apiInventory && apiInventory.length
			? apiInventory.map((i: any) => ({
					id: i.id,
					name: i.name,
					...
					<Card className="p-4 sm:p-6 hover:shadow-lg transition-all border border-pink-100 hover:border-pink-400 dark:border-pink-900 dark:hover:border-pink-400 shadow-xl rounded-2xl bg-white dark:bg-gray-950">
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
							{services.map((service) => (
								...
								>
									<p className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2 group-hover:text-pink-600 dark:group-hover:text-pink-400">
										{service.name}
									</p>
						...
						<div className="space-y-3 mb-6 max-h-[400px] overflow-y-auto no-scrollbar">
							{cart.length === 0 ? (
								...
								cart.map((item) => (
									<div
										key={item.id}
										className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-pink-200 dark:hover:border-pink-900/50 transition-all"
									>
						...
						<Card className="p-4 sm:p-6 hover:shadow-lg transition-all border border-green-100 dark:border-green-900 shadow-xl rounded-2xl bg-linear-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20">
							...
							<div className="space-y-3 mb-6">
								{paymentMethods.map((pm, index) => (
									<div
										key={index}
										className="flex items-center gap-3 bg-white dark:bg-gray-950 p-4 rounded-2xl border border-green-100 dark:border-green-900/50 shadow-sm"
									>
										<Badge
											className={`${
												pm.type === "cash"
										...
										<Input
											...
											value={pm.amount}
											onChange={(e) =>
												updatePaymentAmount(index, Number(e.target.value))
											}
							...
							<div className="space-y-4 mb-8 bg-white/50 dark:bg-black/10 p-5 rounded-2xl border border-white/50 dark:border-white/5">
								...
								>
									<span className="text-lg  uppercase tracking-wider">
										{remaining > 0 ? "Reste à payer:" : "Monnaie:"}
									</span>
									<span className="text-2xl font-black">
										{Math.abs(remaining).toLocaleString()} CDF
									</span>
								</div>
							</div>
