...
export function AppointmentResults({ data }: { data: Appointment[] }) {
	...
	const [selectdAppointment, setSelectedAppointment] =
	...
	const filteredAppointments = data?.filter(
		(a: any) => a.status === statusFilter || !statusFilter,
	);

	const getPaymentIntent = (transactionId: string) => {
		return paymentIntents?.find(
			(intent: any) => intent.transactionId === transactionId,
		);
	};
			...
			{filteredAppointments?.map((appointment: any) => {
				...
				return (
					...
					>
						...
						) : appointment.sale.payments[0]?.method ? (
							<div className="mt-3 text-sm text-gray-500">
								Paiement par{" "}
								<span className="text-white">
									{appointment.sale.payments[0]?.method} Balance{" "}
								</span>
								Montant Paye :{" "}
								<span className="text-white">
									{appointment.sale.payments[0]?.amount} CDF
								</span>
							</div>
						) : (
							<div className="mt-3 text-sm text-gray-500">
