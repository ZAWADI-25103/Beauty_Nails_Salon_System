...
import {
	...
	requireRole,
	successResponse,
} from "@/lib/api/helpers"; // Reusing helpers from your existing route.ts
	...
	try {
		...
		const responseData = {
			...
			lastCommissionPaidAt: workerProfile.lastCommissionPaidAt
				? workerProfile.lastCommissionPaidAt.toISOString()
				: null,
		};
...
export async function PUT(
	...
	try {
		...
		if (commissionFrequency !== undefined)
			updateData.commissionFrequency = commissionFrequency;
		...
		if (lastCommissionPaidAt !== undefined)
			updateData.lastCommissionPaidAt = lastCommissionPaidAt
				? new Date(lastCommissionPaidAt)
				: null;
		...
		const responseData = {
			...
			lastCommissionPaidAt: updatedWorkerProfile.lastCommissionPaidAt
				? updatedWorkerProfile.lastCommissionPaidAt.toISOString()
				: null,
		};
