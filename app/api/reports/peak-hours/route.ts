	...
	try {
		...
		appointments.forEach((a: any) => {
			...
			if (a.service?.category) {
				categoryBreakdown[a.service.category] =
					(categoryBreakdown[a.service.category] || 0) + 1;
			}
		});

		const peakHours = Object.entries(buckets).map(([hour, bookings]) => ({
			hour,
			bookings,
		}));
		// Sort by hour
			...
			const sections: ContentSection[] = [
				...
				},
				{
					...
					data: {
						...
						rows: peakHours.map((ph) => [ph.hour, ph.bookings.toString()]),
					},
				},
				{
