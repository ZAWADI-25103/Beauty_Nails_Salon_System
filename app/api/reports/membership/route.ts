...
export async function GET(request: NextRequest) {
	try {
		...
		const totalMembers = await prisma.membershipPurchase.count({
			where: { status: "active" },
		});
		...
		const memberRevenue = totalSales
			.filter((s) => memberIds.includes(s.clientId))
		...
		return successResponse({
			...
			memberRevenue,
			nonMemberRevenue,
			averageMemberSpend: memberIds.length
				? Math.round(memberRevenue / Math.max(1, memberIds.length))
				: 0,
