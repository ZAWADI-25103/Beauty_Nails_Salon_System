...
export async function GET(request: NextRequest) {
	try {
		...
		const prevClientIds = new Set(
			prevPeriodClients.map((appt) => appt.clientId),
		);
		...
		const currentPeriodClients = new Set(
			completedAppointments.map((appt) => appt.clientId),
		);
		const retainedClients = Array.from(prevClientIds).filter((id) =>
			currentPeriodClients.has(id),
		);

		// Rate = (Retained / Total who visited in previous period)
		const retentionRate =
			prevClientIds.size > 0
				? (retainedClients.length / prevClientIds.size) * 100
				: 10;

		// Calculate client performance metrics
		const clientStats = completedAppointments.reduce(
			(acc: any, appointment: any) => {
				...
				if (!acc[clientId]) {
					acc[clientId] = {
						...
						totalRevenue: appointment.client?.totalSpent
							? Number(appointment.client.totalSpent)
							: 0,
					};
				}
		...
		const topClients = [
			...new Set([...sortedByAppointments, ...sortedByRevenue]),
		].slice(0, 10);
