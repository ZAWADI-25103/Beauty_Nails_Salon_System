...
export async function POST(request: NextRequest) {
  try {
    ...
    const packageDuration = servicePackage.services.reduce((sum, service) => sum + service.duration, 0);
    ...
    // Note: Package discount is already applied in servicePackage.price
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const packageDiscountRate = servicePackage.discount > 0 ? servicePackage.discount / 100 : 0;
