...
import { type TasksParams, tasksApi } from "../api/tasks";
...
export function useCreateTask() {
	...
	const m = useMutation({
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.error?.message || "Error creating task",
			);
		},
	});
...
export function useUpdateTask() {
	...
	const m = useMutation({
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.error?.message || "Error updating task",
			);
		},
	});
...
export function useAssignTask() {
	...
	const m = useMutation({
		mutationFn: ({ id, workerId }: { id: string; workerId: string }) =>
			tasksApi.assignTask(id, workerId),
		onSuccess: (data) => {
			qc.invalidateQueries({ queryKey: ["tasks"] });
			qc.invalidateQueries({ queryKey: ["tasks", data.task.id] });
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.error?.message || "Error assigning task",
			);
		},
	});
...
export function useDeleteTask() {
	...
	const m = useMutation({
		...
		onError: (error: any) => {
			toast.error(
				error.response?.data?.error?.message || "Error deleting task",
			);
		},
	});
