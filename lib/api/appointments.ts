import type { PaymentIntent, Review, Service } from "@/prisma/generated/client";
...
export const appointmentsApi = {
	...
	rescheduleAppointment: async (
		...
		const { data } = await axiosdb.patch(
	...
	getAvailableSlots: async (
		...
		const { data } = await axiosdb.get("/appointments/available-slots", {
			params,
		});
		return data;
	},
	...
	sendReminder: async (
		...
		const { data } = await axiosdb.post(`/appointments/${id}/reminder`, {
			type,
		});
		return data;
	},
	...
	): Promise<{ transfer: AppointmentTransfer; message: string }> => {
	...
	respondToTransfer: async (
		...
		const { data: response } = await axiosdb.patch(
			`/appointments/${appointmentId}/transfer`,
			{ action, notes },
		);
		return response;
	...
	getTransfer: async (
		appointmentId: string,
	): Promise<AppointmentTransfer | null> => {
		try {
